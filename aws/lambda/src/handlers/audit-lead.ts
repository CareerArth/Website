...
import { sendAuditEmails } from '../shared/email';
...
import { auditLeadSchema } from '../shared/validation';
...
export const handler = async (event: APIGatewayProxyEventV2) => {
  ...
  try {
    const payload = auditLeadSchema.parse(rawBody);
    ...
    const rateLimit = await incrementRateLimit('audit-lead', sourceIpHash);
    ...
    const stored = await storeSubmission('audit_lead', {
      ...record,
      sourceIpHash,
    ...
    await sendAuditEmails(payload.email, payload.fullName, stored.id);
